...
"""
   ...
   Users should just use ants_lct.py instead.
"""
...
def qa_methods_segment(ants_lct_info):

     output_segment_files   = [[ ants_lct_info['input']['t1'],':visible=1:colormap=grayscale'],
                               [ ants_lct_info['segment']['p_csf'], ':visible=0:colormap=heat:heatscale=0.1,0.5,1:opacity=0.5'],
                               [ ants_lct_info['segment']['p_gm_cortical'],':visible=0:colormap=heat:heatscale=0.1,0.5,1:opacity=0.5'],
                               [ ants_lct_info['segment']['p_wm'], ':visible=0:colormap=jet:colorscale=0.2,0.1:opacity=0.5'],
                               [ ants_lct_info['segment']['segment'], ':visible=1:colormap=lut:colorscale=0,6:opacity=0.75:lut='+ 
...
def qa_results(output_files, ants_lct_info, verbose=True):

     if verbose:
         json_utils.print_json_redcap_instrument(ants_lct_info['results']['json'])
...
def qa_display(output_files, method):
     ...
     if qa.qa_exist(output_files, False):
          ...
          print('Unable to QA ' + method + ' ants_lct.py')
...
def get_info(input_dir, subject_id=None, methods_dir='../methods', results_dir='../results',
    ...
    results_files = OrderedDict(( ('json', os.path.join(base['results'], 'ants_lct.json')),
    ...
    ants_lct_info = {'base': base, 'input': input_files, 'extract': extract_files,
            'segment':segment_files, 'register':register_files, 'results':results_files}

    return ants_lct_info

#endregion

def status_methods( ants_lct_info, verbose=False, debug=False ):

     register_files =  [ value for key, value in ants_lct_info['register'].items() ]
        
     ants_lct_status = util.check_files(register_files,debug)

     if verbose:
          print( ants_lct_info['base']['input'] + ', ants_lct, ' + 'methods' + ', ' + str(ants_lct_status) )
...
def ants_lct_status( out_directory, out_prefix):
...
def results(ants_lct_info, verbose=False):

     # Create results directory and populate

     util.mkcd_dir( [ ants_lct_info['base']['results'] ], True )

     files_to_link = [ ( ants_lct_info['segment']['p_wm'], ants_lct_info['results']['p_wm']),
                       ( ants_lct_info['segment']['segment'], ants_lct_info['results']['segment']),
                       ( ants_lct_info['register']['ct'], ants_lct_info['results']['ct']), 
                       ( ants_lct_info['register']['subject_to_template_affine'], ants_lct_info['results']['subject_to_template_affine']),
                       ( ants_lct_info['register']['subject_to_template_warp'], ants_lct_info['results']['subject_to_template_warp']),
                       ( ants_lct_info['register']['subject_to_template_jacobian'], ants_lct_info['results']['subject_to_template_jacobian']),
                       ( ants_lct_info['register']['template_to_subject_affine'], ants_lct_info['results']['template_to_subject_affine']),
                       ( ants_lct_info['register']['template_to_subject_warp'], ants_lct_info['results']['template_to_subject_warp'])
                       ]

     for ii in files_to_link:
          util.force_hard_link( ii[0],ii[1])

     fsl_command = ['fslmaths', ants_lct_info['segment']['p_gm_cortical'], '-add',  ants_lct_info['segment']['p_gm_subcortical'],
                    ants_lct_info['results']['p_gm']]

     util.iw_subprocess(fsl_command)

     methods_write_json_redcap_ants_lct_instrument(ants_lct_info, verbose)
     

def methods_write_json_redcap_ants_lct_instrument(ants_lct_info, verbose):
    """ Write ANTs CT Instrument to JSON output file"""

    df_stats = labels.properties(ants_lct_info['segment']['segment'])

    dict_redcap = OrderedDict((('subject_id', ants_lct_info['base']['subject_id']),
                               ('ants_lct_analyst', getpass.getuser()),
                               ('ants_lct_datetime', '{:%Y-%b-%d %H:%M:%S}'.format(datetime.datetime.now())),
                               ('ants_lct_csf_volume_mm3', '{0:4.3f}'.format(df_stats.iloc[0]['volume_mm3'])),
                               ('ants_lct_gm_cortical_volume_mm3', '{0:4.3f}'.format(df_stats.iloc[1]['volume_mm3'])),
                               ('ants_lct_wm_volume_mm3', '{0:4.3f}'.format(df_stats.iloc[2]['volume_mm3'])),
                               ('ants_lct_gm_subcortical_volume_mm3','{0:4.3f}'.format(df_stats.iloc[3]['volume_mm3'])),
                               ('ants_lct_brainstem_volume_mm3','{0:4.3f}'.format(df_stats.iloc[4]['volume_mm3'])),
                               ('ants_lct_cerebellum_volume_mm3', '{0:4.3f}'.format(df_stats.iloc[5]['volume_mm3']))
                               )
                              )

    with open( ants_lct_info['results']['json'], 'w') as outfile:
...
if __name__ == '__main__':
     ...
     parser = argparse.ArgumentParser(prog='ants_lct')
     parser.add_argument('--t1full',          help='Full T1w image (default = t1w.nii.gz )', nargs='*', default = 't1w.nii.gz' )
     ...
     # Grab info
     ants_lct_info = get_info( os.path.abspath(inArgs.indir), subject_id=inArgs.subject_id, methods_dir=inArgs.outdir,
                             results_dir=inArgs.results_dir, t1=inArgs.t1full, t2=inArgs.t2full, flair=inArgs.t2flair,
                             verbose=inArgs.verbose)

     out_directory = ants_lct_info['base']['methods']
     ...
     output_extract_files =  [[ ants_lct_info['input']['t1'],':visible=1:colormap=grayscale'],
                              [ ants_lct_info['extract']['mask'], ':visible=1:colormap=jet:opacity=0.5']
                              ]
     ...
     output_result_files   = [[ ants_lct_info['input']['t1'],':visible=1:colormap=grayscale'],
                              [ ants_lct_info['results']['p_gm'], ':visible=0:colormap=heat:heatscale=0.1,0.5,1:opacity=0.5'],
                              [ ants_lct_info['results']['p_wm'], ':visible=0:colormap=heat:heatscale=0.1,0.5,1:opacity=0.5'],
                              [ ants_lct_info['results']['ct'],':visible=0:colormap=heat:opacity=0.75'],        
                              [ ants_lct_info['results']['segment'], ':visible=1:colormap=lut:colorscale=0,6:opacity=0.75:lut='+ 
                                brain_segmentation_lut],                             
                               ]
     ...
     if inArgs.mask is not None:
          ...
          if not os.path.exists(out_directory):
               ...
               if ants_lct_status( out_directory, inArgs.outprefix):
                    print('\nThe output directory should be cleaned before proceeding \n'
                          'or a new output directory should be set. \n')
                    exit()               
               
          shutil.copy2( util.path_relative_to(ants_lct_info['base']['input'], inArgs.mask ), ants_lct_info['extract']['mask'] ) 
     ...
     if 'segment' in inArgs.qa or 'methods' in inArgs.qa:
          qa_methods_segment(ants_lct_info)
     ...
     if inArgs.results:
          results(ants_lct_info, inArgs.verbose)

     # Status

     if 'methods' in inArgs.status  or 'all' in inArgs.status:
          status_methods(ants_lct_info, True, inArgs.debug)

     if 'results' in inArgs.status  or 'all' in inArgs.status:
          status_results(ants_lct_info, True, inArgs.debug)

     # QA Results
     #

     if 'results' in inArgs.qa:
          qa_results(output_result_files, ants_lct_info, inArgs.verbose)
